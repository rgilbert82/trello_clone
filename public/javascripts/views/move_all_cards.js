var MoveAllCardsView = Backbone.View.extend({
  ...
  moveAllCards: function(e) {
    ...
    this.model.get("cards").slice().forEach(function(card) {
      ...
      card.set({ "listID": newListID, "position": length + 1 });
    });

    App.reSetupBoard(this.model.get("boardID"));
  },
  setupDropdown: function() {
    var $listsDropdown = this.$el.find(".select_list_list").find("optgroup");
    var listID = this.model.get("id");
    var otherLists = App.lists.toJSON().filter(function(list) { return list.id !== listID });
